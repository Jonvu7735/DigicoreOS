//! reporting-svc entrypoint.
//!
//! Responsibilities (ARCHITECTURE.md §3.5): subscribe to platform events, build
//! fact/snapshot read models, and serve dashboards/reports. Boot order: load
//! config -> init observability -> wire dependencies -> build router -> serve
//! with graceful shutdown.

// Skeleton phase: scaffold (config, wiring, auth, outbox relay) is in place; the
// event consumer + read-model slices fill in without restructuring.
#![allow(dead_code)]
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(%addr, "reporting-svc listening");
    ...
    tracing::info!("reporting-svc shut down cleanly");
    Ok(())
}
