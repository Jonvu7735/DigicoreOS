//! core-erp-svc entrypoint.
//!
//! Responsibilities (SERVICE-core-erp-svc.md): orders, inventory, products,
//! invoices, payments. Boot order: load config -> init observability -> wire
//! dependencies -> build router -> serve with graceful shutdown.

// Skeleton phase: types/ports are declared ahead of first use so domain slices
// (orders, inventory, ...) can be filled in without restructuring.
#![allow(dead_code)]
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(%addr, "core-erp-svc listening");
    ...
    tracing::info!("core-erp-svc shut down cleanly");
    Ok(())
}
