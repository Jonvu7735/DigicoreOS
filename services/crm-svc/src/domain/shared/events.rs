//! Convert a domain `CrmEvent` into an outbox message (header + JSON payload).

use event_models::crm::CrmEvent;
use platform_outbox::OutboxMessage;

use crate::domain::shared::error::{DomainError, DomainResult};

pub fn outbox_message(event: &CrmEvent) -> DomainResult<OutboxMessage> {
