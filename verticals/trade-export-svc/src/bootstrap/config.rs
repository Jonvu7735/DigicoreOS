//! Environment-based configuration for trade-export-svc.
//!
//! Every value has a safe local-dev default. Production values come from
//! env/secrets (the shared `digicore-config` ConfigMap + `digicore-jwt` secret).

use anyhow::Context;

pub const SERVICE_NAME: &str = "trade-export-svc";
...
pub struct AppConfig {
    ...
    /// Dedicated schema; this vertical owns ONLY `trade_export_svc`.
...
/// JWT verification settings. The vertical only VERIFIES the tokens auth-svc
/// issues, so it holds the public key (AUTH-FLOW.md §7).
#[derive(Debug, Clone)]
...
impl AppConfig {
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        Ok(Self {
            ...
            env: env.clone(),
            http_port: env_or("HTTP_PORT", "8087")
                .parse()
            ...
            database_schema: env_or("DATABASE_SCHEMA", "trade_export_svc"),
...
/// `JWT_PUBLIC_KEY_PATH`, then (dev only) `./.dev/jwt_public.pem`.
