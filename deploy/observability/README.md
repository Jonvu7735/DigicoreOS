...
| `alerts.yaml` | `PrometheusRule` — service-down, auth login/refresh failure spikes, event-consumer failures |
| `dashboards/digicore-overview.json` | Grafana dashboard: service availability, auth login/refresh rates, event throughput |
...
Emitted today (see `libs/platform-observability` + the services): `up` (from
scraping), `auth_login_success_total` / `auth_login_failed_total`,
`auth_refresh_success_total` / `auth_refresh_failed_total`,
`auth_register_success_total`, `events_published_total`, `events_consumed_total`,
`events_consumed_failed_total`.

> **Next enrichment**: `OBSERVABILITY.md §4.3` also prescribes standard HTTP metrics
> (`http_requests_total{service,method,path,status}`,
> `http_request_duration_seconds_bucket`). Those aren't emitted yet (the HTTP layer
> currently produces tracing spans, not Prometheus series); adding a small metrics
> middleware to `platform-observability` would light up latency/error-rate panels
> here. Tracing already exports to OTLP (Jaeger/Tempo) when
