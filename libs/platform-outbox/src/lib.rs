...
//! - [`RawEventPublisher`] / [`JetStreamPublisher`]: ship messages to the bus
//!   (JetStream — durable, server-ACKed, deduplicated by event id).
...
pub use nats::{
    connect_publisher, ensure_stream, JetStreamPublisher, STREAM_NAME, STREAM_SUBJECTS,
};
